...
class FaceRecogniser(object):
    ...
    def file_recognize_faces(self,file):
        kisi=[]
        con = []
        for img in os.listdir(file):
            ...
            kisi.append({'name': person1, 'confidence': confidence1})
            con.append(confidence1)
        return kisi
        ...
        #     con.sort()
        #
        #     diff = [y - x for x, y in zip(*[iter(con)] * 2)]
...
if __name__ == "__main__":
    ...
    for f in os.listdir("output/"):
        # print f
        p = "output/" + f
        sonuc = a.file_recognize_faces(p)
        dizi.append({"dosya":f, "sonuc:" :sonuc})
    print(len(dizi))
    print("*******************SONUCLAR*************************************")
    for d in dizi:
        print d
    # a.file_recognize_faces("output/rte_deb1/")